...
function useScreenSize(): { isMobileScreen: boolean; isTabletScreen: boolean } {
  const [isMobileScreen, setIsMobileScreen] = useState<boolean>(window.matchMedia('(max-width: 48em)').matches);

  const [isTabletScreen, setIsTabletScreen] = useState<boolean>(window.matchMedia('(max-width: 62em)').matches);
